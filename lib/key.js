...
Key = (function() {
  ...
  Key.decryptKeys = function(keys, wmid, password) {
  ...
  Key.checkData = function(header, keys) {
    var crc;
  ...
  Key.fromFile = function(fileName, wmid, password) {
    ...
    fs.closeSync(file);
    this.decryptKeys(keys, wmid, password);
    this.checkData(header, keys);
    return this.parseKeys(keys);
  };
